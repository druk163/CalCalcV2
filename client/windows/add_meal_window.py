...
class AddMealWindow(QWidget):
        ...
        try:
            ...
            for p in products:
                ...
                self.products_table.setItem(
                    row, 1, QTableWidgetItem(str(p["calories"]))
                )
                self.products_table.setItem(
                    row, 2, QTableWidgetItem(str(p["proteins"]))
                )
                self.products_table.setItem(
                    row, 3, QTableWidgetItem(str(p["fats"]))
                )
                self.products_table.setItem(
                    row, 4, QTableWidgetItem(str(p["carbs"]))
                )
    ...
    def add_item_to_list(self):
        ...
        product_id = self.products_table.item(selected, 0).data(
        ...
        self.selected_table.setItem(
            row, 0, QTableWidgetItem(product_name)
        )
        self.selected_table.setItem(
            row, 1, QTableWidgetItem(str(weight))
        )

        btn_remove = QPushButton("X")
        btn_remove.clicked.connect(
            lambda checked, r=row: self.remove_item(r)
        )
    ...
    def save_meal(self):
        if not self.selected_items:
            QMessageBox.warning(
                self, "Ошибка", "Добавьте хотя бы один продукт"
            )
            return
